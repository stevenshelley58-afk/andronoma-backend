...
app.post('/api/v1/audiences/generate', async (req, res) => {
  try {
    ...
    console.log(`🎯 Generating ${numPersonas} personas...`);
    
    const prompt = `You are an expert Meta Ads audience strategist. Generate ${numPersonas} distinct audience personas for:

Business: ${businessDescription}
Product/Service: ${productService}
Goals: ${targetGoals || 'Not specified'}
...
[
  {
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(`Google Error: ${error.error?.message || 'Unknown'}`);
      }
    ...
    console.log(`✅ Generated ${newPersonas.length} personas ($${cost.toFixed(4)})`);
